...
def main_loop():
    ...
    for i_iter in range(args.max_iter_num):
        ...
        if i_iter % args.log_interval == 0:
            print('{}\tT_sample {:.4f}\tT_upd {:.4f}\tR_min {:.2f}\tR_max {:.2f}\tR_avg {:.2f}\tM_avg {:.2f}\tW_avg {:.2f}\tM_steps {:.2f}\tW_steps {:.2f}'.format(
        ...
        if args.save_model_interval > 0 and (i_iter+1) % args.save_model_interval == 0:
            ...
            pickle.dump((policy_mgr, policy_wrk, value_mgr, value_wrk, running_state),
                        open(os.path.join(assets_dir(), 'learned_models/{}_trpo_dense.p'.format(args.env_name)), 'wb'))
